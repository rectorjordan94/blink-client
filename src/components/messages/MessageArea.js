    ...
    if (!currentChannel) {
        return (
            <div className="container w-75 d-flex justify-content-center align-items-center" id="info-container">
                <div className="card mx-5" id="info-card">
                    <div className="card-header text-center" id="info-header">
                        BLINK
                    </div>
                    <div className="card-body" id="info-body">
                        <h5 className="card-title text-center mb-3" id="info-username">Hi, <span>{ profile ? profile.username : user.email}</span>!</h5>
                        <p className="card-text"><span className="info-spans">Welcome to BLINK</span>, a Slack clone project where users can create and join different channels and chat with other users in real-time.</p>
                        <p className="card-text"><span className="info-spans">For first time users</span>, it is recommend to start by selecting the <span className="info-spans">Create Profile</span> link in the navigation bar so that you can create your own username, add your pronouns, and set your location.</p>
                        <p className="card-text"><span className="info-spans">If this isn't your first rodeo</span>, or if you prefer to use your email as your username, feel free to select one of your channels from the sidebar to start chatting.</p>
                        <p className="card-text"><span className="info-spans">Don't see the channel you're looking for?</span> You can search our entire database of channels using the search bar at the top of the page, or click the <span className='info-spans'>+</span> in the sidebar to create your own and invite other users to join!</p>
                        <p className="card-text">This is a project built with the MERN stack, using the MVC system for organizing the code. This project also utilizes socket.io to broadcast events and receive responses, which allows all user clients to update when changes are made in real-time. To view this project's code or to see any of my other projects click <a href="https://github.com/rectorjordan94" className="info-spans">here</a>.</p>
                    </div>
                    <div className="card-footer text-muted d-flex justify-content-end" id="info-footer">
    ...
    const threadListItems = threads.map((thread, i) => (
        <a href="#" className="list-group-item list-group-item-action channel-threads" onClick={onClick} id={thread._id} key={i}>
                <div style={{pointerEvents: 'none'}} className="d-flex w-100 justify-content-between align-items-center">
                    <h5 className="thread-username" style={{ pointerEvents: 'none' }}>{thread.author.username ? thread.author.username : thread.owner.email}</h5>
